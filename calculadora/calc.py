...
class Calculator:
    def __init__(self, master):
        ...
        Button(width=11, height=4, text="C", relief="flat", bg="#F0F8FF", command=self.clear).place(x=270, y=50)
        Button(width=11, height=4, text="x", relief="flat", bg="white", command=lambda: self.show("*")).place(x=270, y=125)
        Button(width=11, height=4, text="=", relief="flat", bg="#60A5FA", command=self.solve).place(x=270, y=350)
        Button(width=11, height=4, text="/", relief="flat", bg="white", command=lambda: self.show("/")).place(x=0, y=350)


    def show(self, value):
        if self.equation.get() == "Erro":
            self.entry_value=""
    ...
    def solve(self):
        try:
            equation_to_eval = self.entry_value.replace("%", "/100")
